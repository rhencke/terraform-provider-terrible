...
- `chain_management` (Boolean) If `true` and `state` is `present`, the chain will be created if needed. If `true` and `state` is `absent`, the chain will be deleted if the only other parameter passed are `chain` and optionally `table`.
...
- `ctstate` (String) A list of the connection states to match in the conntrack module. Possible values are `INVALID`, `NEW`, `ESTABLISHED`, `RELATED`, `UNTRACKED`, `SNAT`, `DNAT`.
...
- `gateway` (String) This specifies the IP address of the host to send the cloned packets. This option is only valid when `jump=TEE`.
...
- `log_level` (String) Logging level according to the syslogd-defined priorities. The value can be strings or numbers from 1-8. This parameter is only applicable if `jump=LOG`.
...
- `match_set` (String) Specifies a set name that can be defined by ipset. Must be used together with the `match_set_flags` parameter. When the `!` argument is prepended then it inverts the rule. Uses the iptables set extension.
- `match_set_flags` (String) Specifies the necessary flags for the match_set parameter. Must be used together with the `match_set` parameter. Uses the iptables set extension. Choices `dst,dst` and `src,src` added in version 2.17.
...
- `policy` (String) Set the policy for the chain to the given target. Only built-in chains can have policies. This parameter requires the `chain` parameter. If you specify this parameter, all other parameters will be ignored. This parameter is used to set the default policy for the given `chain`. Do not confuse this with `jump` parameter.
